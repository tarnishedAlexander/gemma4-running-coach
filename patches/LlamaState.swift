...
class LlamaState: ObservableObject {
    ...
    private let defaultModels: [Model] = [
        Model(
            name: "Gemma 4 E2B (Q4_K_M, 2.9 GiB) [bundled]",
    ...
    func loadModel(modelUrl: URL?) throws {
        if let modelUrl {
            ...
            // Auto-load bundled vision projector (mmproj) if present
            if let mmprojUrl = Bundle.main.url(forResource: "mmproj-F16", withExtension: "gguf", subdirectory: "models") {
                Task {
                    do {
                        try await llamaContext?.loadMmproj(path: mmprojUrl.path())
                        await MainActor.run { self.messageLog += "Loaded mmproj (vision) \(mmprojUrl.lastPathComponent)\n" }
                    } catch {
                        await MainActor.run { self.messageLog += "Vision mmproj load failed: \(error)\n" }
                    }
                }
            }
    ...
    /// Run a completion with optional image attachment.
    func complete(text: String, imagePath: String?) async {
        guard let llamaContext else { return }
        let t_start = DispatchTime.now().uptimeNanoseconds
        if let imagePath {
            await llamaContext.completion_init_with_image(text: text, imagePath: imagePath)
        } else {
        ...
        Task.detached {
            while await !llamaContext.is_done {
                ...
                await MainActor.run { self.messageLog += "\(result)" }
            }
            let t_end = DispatchTime.now().uptimeNanoseconds
            let t_total = Double(t_end - t_start) / self.NS_PER_S
            let n_decode = await llamaContext.n_decode
            let tps = Double(n_decode) / (t_total - t_heat)
            await MainActor.run {
                self.messageLog += "\n\nDone\nPrefill: \(String(format: "%.2f", t_heat))s   Decode: \(String(format: "%.1f", tps)) tok/s\n"
            }
